...
<div class="container-fluid navbar-dark" style="background: #ffffff; color: black;">
    {{-- @auth
            ...
            <ul class="list-group">
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    @switch($notification->data['type'])
                        @case('comment')
                            <a href="{{$notification->data['url']}}">{{$notification->data['commentedUsername']}} commented on your post: {{$notification->data['title']}}</a>
                            <span class="badge badge-primary badge-pill" style="background: #e29fee">{{$notification->data['commentedUsername']}}</span>
                            @break
                        @case('audio')
                            <a href="{{$notification->data['url']}}">{{$notification->data['commentedUsername']}} commented with voice note on your post: {{$notification->data['title']}}</a>
                            <span class="badge badge-primary badge-pill" style="background: #e29fee">{{$notification->data['commentedUsername']}}</span>
                            @break
                        @case('reply')
                            <a href="{{$notification->data['url']}}">{{$notification->data['commentedUsername']}} replied your comment: {{$notification->data['title']}}</a>
                            <span class="badge badge-primary badge-pill" style="background: #e29fee">{{$notification->data['commentedUsername']}}</span>
                            @break
                    @endswitch
                </li>
        ...
        @empty
            <strong>U do not have any notification yet</strong>
        @endforelse
    @endauth --}}
    ...
    <header class="blog-header py-1">
        <div class="row flex-nowrap justify-content-between align-items-center">
            <div class="col-2 pt-1">
                <a class="navbar-brand link-mirror text-dark" href="{{ url('/') }}">
                    <!-- {{ config('app.name', 'MirorLog') }} -->
                    <img src="{{ asset('/storage/sounds/mirror2.png') }}">
                </a>
            </div>
            <div class="col-8">
                {{--
                    <a class="blog-header-logo link-mirror nav-link text-dark" id="post_content_nav" href="#">POST</a>
                    ...
                    <button type="button" class="btn btn-outline-mirror" id="show_add_contents"
                ...
                <div class="TopHeader">
                    <div class="InputBox">
                        <input type="search" placeholder="Search MirrorLog" id="SearchInput">
            ...
            <div class="col-2 d-flex justify-content-end align-items-center">
                @auth
                    <li class="navbar-nav nav-item dropdown">
                        <a href="#" id="UserNameDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            @if (Storage::exists("public/profile/$logged_in_user->username/profile_location/$logged_in_user->profile_location"))
                                {{-- <img src="/storage/profile/{{Auth::user()->username}}/profile_location/{{Auth::user()->profile_location}}"
                                        alt="{{ Auth::user()->username }} - MirrorLog" class="user__photo"
                                ...
                                @else
                                    <div class="container" style="position: relative">
                                        <img src="{{ asset('images/default_user picture icon.png') }}"
                                            alt="MirrorLog default profile pix" class="user__photo"
                                            style="border-radius: 50%; height: 1.5rem; width: 1.5rem; margin-right: 0.2rem">
                                        @if (Auth::user()->unreadNotifications->count())
                                            <span class="badge" style="background:#3d1540; position:absolute;
                                                    top: -2px; left: 36px; font-size:10px; border-radius: 50%">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="UserNameDropdown">
                            <li>
                                <a class="dropdown-item" href="{{ route('viewprofile', $logged_in_user) }}">
                                    @if (Storage::exists("public/profile/$logged_in_user->username/profile_location/$logged_in_user->profile_location"))
                                        <img src="/storage/profile/{{Auth::user()->username}}/profile_location/{{Auth::user()->profile_location}}"
                                                alt="{{ Auth::user()->username }} - MirrorLog" class="user__photo"
                                                style="border-radius: 50%; height: 1.5rem; width: 1.5rem; margin-right: 0.2rem">
                            ...
                            @if (Auth::user()->unreadNotifications->count())
                                <li>
                                    <a class="dropdown-item" href="{{ route('notification') }}"><i class="fa fa-solid fa-bell"></i>
                                        {{ Auth::user()->unreadNotifications->count() <=9  ? Auth::user()->unreadNotifications->count() : '9+' }} Notification
                                    </a>
                                </li>
                            @else
                                <li>
                                    <a class="dropdown-item" href="{{ route('notification') }}"><i class="fa fa-solid fa-bell"></i> 0 Notification</a>
                                </li>
                            @endif
                            <li>
                                <a class="dropdown-item" href="{{ route('logout') }}"><i class="fa fa-solid fa-door-open"></i> Logout</a>
                            </li>
                            <li class="dropdown-divider"></li>
                            <li>
                                <a class="dropdown-item" href="{{ route('about') }}"><i class="fa fa-solid fa-book"></i> About MirrorLog</a>
                            </li>
                        </ul>
                    </li>
                @endauth
                ...
                @guest
                    <li class="navbar-nav nav-item dropdown">
                        ...
                        <!-- <a class="link-mirror text-dark nav-link dropdown-toggle" href="" id="LoginDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false"> -->
                            <!-- My Account -->
                            <img src="{{ asset('images/default_user picture icon.png') }}"
                                    alt="MirrorLog default profile pix" class="user__photo"
                                    style="border-radius: 50%; height: 1.5rem; width: 1.5rem; margin-right: 0.2rem">
